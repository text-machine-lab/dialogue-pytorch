...
Added code for multi-gpu cuda compatibility.
"""
import abc
...
class Head(StatefulComponent):
    ...
    def reset(self, batch_size, device=None):
        super().reset(batch_size)
        self.w = Variable(torch.zeros(batch_size, self.memory_size))
        if device is not None:
...
class ReadHead(Head):
    ...
    def hidden_state_splitting_scheme(self):
        return [
            ...
            (1, F.softplus),                            # b
            (1, torch.sigmoid),                             # g
            (2*self.max_shift_size+1, partial(F.softmax, dim=-1)),       # s
...
class WriteHead(Head):
    ...
    def hidden_state_splitting_scheme(self):
        return [
            ...
            (2*self.max_shift_size+1, partial(F.softmax, dim=-1)),       # s
            (1, lambda x: 1 + F.softplus(x)),           # r
            (self.memory_feature_size, torch.sigmoid),      # e
...
class NTM(StatefulComponent):
    ...
    def reset(self, batch_size, device=None):
        ...
        self.memory.reset(batch_size, device=device)
        for read_head, write_head in zip(self.read_heads, self.write_heads):
            read_head.reset(batch_size, device=device)
            write_head.reset(batch_size, device=device)
